...
description: Loop find→judge→apply simplification rounds over a scope (uncommitted | branch | unpushed | codebase) via a Workflow until fresh finders come up empty, optionally followed by a comment-pruning phase.
disable-model-invocation: true
---

A Workflow converges the scoped code to a stable simplified form: rounds of find→judge→apply over batches of files — fresh finders propose, independent judges strike what is not a genuine improvement, appliers implement only what survives — until a confirmation sweep applies nothing and the tree hash lands on an already-seen state. When pruning is requested, a comment-pruning phase follows convergence. You orchestrate from outside the loop: resolve the inputs, launch the workflow, relay its result. The loop mechanics, embedded prompts, and comment-classification rules live in [simplify.mjs](simplify.mjs) in this skill's folder.
...
1. **Resolve the arguments.** Parse them in this exact order; every "ask the user" below means ask and stop, without launching anything.
   - Remove the word `prune` from the argument list wherever it appears — its presence enables the comment-pruning phase.
   - The first remaining argument is the scope and must be exactly one of `uncommitted`, `branch`, `unpushed`, `codebase`; if it is missing or anything else, ask the user which scope to use.
   - After the scope, what remains must be nothing, `[model]`, or `[model] [effort]` — in that order, nothing else.
   - The model must be a plausible model name (e.g. `opus`, `sonnet`, `haiku`). If the token after the scope is not a model name, ask the user what they meant. If a model is given without a following effort — one of `low`, `medium`, `high`, `xhigh`, `max` — ask the user which effort to use; the model/effort pairing is the user's call, never defaulted.
...
3. **Compute the file list** for the scope, via Bash from the project root. Every entry becomes a file some agent is told to simplify, so the list must contain only source files that still exist: filter it through a `while IFS= read -r f` loop with a `-f` test to drop deleted paths (git quotes paths containing backslash, double-quote, newline or control bytes, so those drop here too and stay out of scope), and exclude anything that is not hand-written source — lockfiles, binaries, vendored or generated code, pure-data files, markdown. The `grep -vE` patterns below are a starting point; adjust them with judgment about the repo at hand (its build output directory, its generated-code conventions, its vendored trees).
   ...
   If the list is empty, report that there is nothing to simplify and stop without launching the workflow — the workflow rejects an empty `files` array.

4. **Resolve the base ref** bounding the diff — `HEAD` for `uncommitted`, the step-2 merge-base for `branch` and `unpushed` (each its own), none for `codebase`. It is required for every non-`codebase` scope, pruning or not.

   **Build the prune candidate lists** (only when pruning is enabled) — only code files that can contain comments (e.g. `.ts`, `.svelte`, `.js`). Neither list may ever name a path step 3's exclusions would have dropped for this scope: a vendored, generated, built or minified file the workflow was told not to simplify must not be reachable by a remove agent either. Tracked candidates are classified diff-bounded, untracked ones whole-file — hence two lists:
   - `pruneFiles` — tracked candidates:
   ...
   - `pruneUntrackedFiles` — untracked candidates, in every scope:
   ...
   An empty list is still passed, as `[]`, and `prune`, `pruneExts`, and `root` with it — even with both lists empty, the workflow classifies the comments the simplify phase itself introduces.

   Also pass `pruneExts` — the extensions you filtered these lists on, e.g. `[".ts", ".js", ".svelte"]`: the set of comment-carrying source extensions for this repo, not narrowed to the extensions that happen to occur in the lists. The workflow judges files that appear mid-run by it.
...
7. **Launch the workflow.** First resolve the absolute path of [simplify.mjs](simplify.mjs) — it sits next to this SKILL.md in the skill's folder, wherever this skill is installed. Test the standard locations via Bash, project install first: `./.claude/skills/simplify-workflow/simplify.mjs` (resolved to an absolute path from the project root), then `~/.claude/skills/simplify-workflow/simplify.mjs`. If neither exists, ask the user where the skill is installed and stop without launching anything. Substitute the resolved path below:
   ...
   Pass `files`, `untrackedBaseline`, `pruneFiles`, `pruneUntrackedFiles`, and `pruneExts` as real JSON arrays, not JSON-encoded strings; `untrackedBaseline` is always required (pass `[]` when the working tree has no untracked files) and `base` is required for every scope except `codebase`, while `prune`, `pruneExts`, `root`, and both prune lists are only needed when pruning is enabled.
...
8. **Report.** Relay `summary` grouped under its three headings (`Performance improvements`, `Code simplifications`, `Bug fixes`), plus the round count (`iterations`), the sweep count (`sweeps`), and the proposed/approved/applied counts (`findingsProposed`, `findingsApproved`, `changesApplied`) — the gap between proposed and approved is the judge doing its job. Relay every entry in `rejectedFindings` — its description, its files, and the judge's reason — under its own heading, so the user sees what the judges struck and why. Leave the changes uncommitted; that holds on every run, pruning or not. Then obey every row of the table below whose condition holds — a row may impose an action or a prohibition, not only a line of report. Three shorthands: a **full prune object** is a `prune` that is neither null nor `{stopReason: "skipped-simplify-unstable"}` — in those two states no other `prune` field exists, so no row that names one applies; **edited** means `changesApplied` is non-zero, or `distinctTreeStates` is non-zero, or a full prune object reports a non-zero `removed` or a non-zero `distinctTreeStates` (`distinctTreeStates` counts the tree states a phase's round hashes reached beyond its starting state — ground-truth proof of an edit even when no agent lived to report one); **edit possible** means `stopReason` is `hash-unavailable`, or a full prune object's `stopReason` is `hash-unavailable` — an applier or remover may have written before that phase lost track of the tree.

   | When | Report |
   | --- | --- |
   | `prune` is null | Say nothing about pruning — it was never requested. No row naming a `prune` field applies. |
   | `prune` is non-null and its `stopReason` is `skipped-simplify-unstable` | State that pruning was skipped because simplification never converged, naming the top-level `stopReason` as what it stopped on. No other row naming a `prune` field applies. |
   ...
   | `unresolvedCheckFailures` is non-empty | Before reporting, fix any breakage yourself in files the workflow touched; then list what still remains, prominently, saying the workflow's own fix-up agent already tried and failed to repair these. An entry whose `file` is `(unattributed)` names no path — the fix-up reported the check still failing without naming any failure — so treat it as a failure with no known location and open no file for it. |
   | **[UNVERIFIED]** Full prune object whose `verificationStatus` is `baseline-failed` or `fixup-died`; or `verificationStatus` is `baseline-failed` or `fixup-died` and `prune` is not a full prune object whose `verificationStatus` is `ran` | State prominently that the project was never verified and that the user should run the check command manually; when `unresolvedCheckFailures` is empty, say that its emptiness proves nothing. |
   ...
   | `verificationStatus` is `fixup-died` and `prune` is a full prune object whose `verificationStatus` is `ran` | Say the mid-run verification died but the final tree was verified — the prune fix-up re-ran the same check on the final tree — so `unresolvedCheckFailures` is meaningful. |
   ...
   | **[NO CHECK]** `verificationStatus` is `not-configured` and (**edited** or **edit possible**) | State prominently that no check command could be found for this project, that an empty `unresolvedCheckFailures` therefore means nothing, and that the user should verify the tree themselves before relying on or committing it. Do not tell them to re-run the check command — there is none. |
   ...
   | **[UNSETTLED]** and the triggering phase recorded a change — top level: `changesApplied` or `distinctTreeStates` non-zero; prune: `prune.removed` or `prune.distinctTreeStates` non-zero | Say the tree was edited. |
   | **[UNSETTLED]** via `max-iterations` and the triggering phase recorded no change — top level: `changesApplied` and `distinctTreeStates` both zero; prune: `prune.removed` and `prune.distinctTreeStates` both zero | Say that nothing was recorded as changed and that the final state could not be confirmed. |
   ...
   | Full prune object whose `stopReason` is `all-batches-clean` | Report it as the expected healthy outcome — every batch was audited until it produced no further candidates — and add no caveat of your own. |
