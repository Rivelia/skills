...
5. **Launch the workflow.** Resolve the absolute path of [review.mjs](review.mjs); it sits next to this SKILL.md, in the directory named by the `Base directory for this skill:` line of this skill's invocation. Substitute the resolved path below:
   ...
   The workflow returns `{scope, implementerModel, base, checksConfigured, finderFailures, clusters, fixes, uncommittedFixFiles, possiblyDirty, findings}`. Each finding carries `status` (`confirmed`, `refuted`, `agent_failed`), `outcome` (`fixed`, `covered`, `not_fixed`, `reverted`, `agent_failed`, or null when never implemented), `claimedSeverity` and settled `severity`, the final `description` with `corrected` set when the technical skeptic rewrote it, `alsoReportedBy`, and the implementer's `plan`, `reason`, `excludedKind`, `notes`, `files`, `kinds`, `hunks`, `checks`, `committed` and `commitSha`.
...
6. **Report** every finding in six categories, ordered by severity within each:
   ...
   - confirmed but not auto-fixed, saying whether the fix was over budget or an excluded kind, with the implementer's plan;
