...
4. **Launch the workflow.** Substitute the resolved paths:
   ...
   - Scout: a Sonnet state agent records `dirtyAtLaunch`, the untracked files and the changed file list from git; a scout on Fable, or the model argument, designs the dimensions from that shape with the same rules the single-run skill gives you. Neither agent is told the round number, an earlier split, or what earlier fixes touched: the prompt is the same on every round and only the tree differs. A file the scout assigns to no dimension gets a catch-all finder, so coverage never depends on the scout.
   - Review: `review.mjs` runs with those dimensions and the round's fresh tree state, the model argument reaching its implementers; its phases, budgets and excluded kinds are its own.
