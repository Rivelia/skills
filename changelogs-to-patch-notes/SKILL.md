...
3. **Write the notes**, in the language of the commit messages.
   ...
   - One `##` heading per release, newest first, with its date; commits past the last release come first under `## Next release`. The headings stay when the range holds a single release.
   - Under each heading, `### New`, `### Improvements`, `### Fixes`, empty ones omitted, bullets ordered by impact on the reader.
   - One bullet per change, one sentence a reader takes in at a glance, spoken to the reader the way a colleague would tell them: it opens with a past-tense verb (Added, Fixed, Improved), names the symptom as the reader saw it and what it cost them, then says what happens now instead. The mechanism behind a bug stays out. Illustrations of the register, not templates to copy: "Fixed attachments vanishing when a message was too long for the model: an error card now appears and the turn stops instead." "Added keyboard shortcuts for moving between conversations, so switching no longer needs the mouse." "Improved search, which now shows results as you type instead of after you press Enter."
   - A bullet is the headline of a change; the details stay in the product for the reader to discover.
   - A new feature too large for one sentence keeps that sentence and adds a few sub-bullets, each a headline of what the reader can now do, as short as the bullet above them. Fixes and improvements stay single lines.
